...
export default function ShareModal({ open, onClose, nodes, edges, log }: ShareModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-6 backdrop-blur-sm">
      ...
      <div className="animate-fade-in-up relative flex w-full max-w-md flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <div>
            <h2 className="text-sm font-bold text-slate-800">Share Diagram</h2>
            <p className="text-[11px] text-slate-400">Read-only link — no source code or secrets included.</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-col items-center gap-4 px-5 py-5">
          ...
          <div className="flex w-full items-center gap-1.5">
            <input
              ...
              className="flex-1 truncate rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-[11px] text-slate-600 outline-none"
            />
            <button
              onClick={handleCopy}
              className="flex shrink-0 items-center gap-1 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[11px] font-semibold text-slate-600 transition-colors hover:bg-slate-50"
            >
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-slate-900 px-3.5 py-2 text-xs font-semibold text-white shadow-sm transition-colors hover:bg-slate-700"
          >
            <Download size={14} />
