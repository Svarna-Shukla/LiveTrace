...
export default function EventLog({ log }: { log: ExecutionStep[] }) {
  return (
    <div className="z-10 flex h-full w-[320px] shrink-0 flex-col border-l border-border bg-white/95 shadow-lg backdrop-blur dark:border-slate-800 dark:bg-slate-950/95">
      <div className="flex items-center gap-1.5 border-b border-border px-3.5 py-2.5 dark:border-slate-800">
        ...
        <span className="ml-auto text-[10px] text-slate-400 dark:text-slate-500">{log.length} events</span>
      </div>
          ...
          <ul className="space-y-1">
            {log.map((step) => (
              <li
                ...
                className={clsx(
                  "animate-fade-in-up rounded-lg border-l-[3px] bg-slate-50/70 px-2.5 py-1.5 dark:bg-slate-900/70",
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span
                    className={clsx(
                      ...
                      step.status === "success" ? "text-emerald-500 dark:text-emerald-400" : "text-red-500 dark:text-red-400",
                    )}
                  >
                    {step.latencyMs}ms
                  </span>
