...
export default function NodeInspectorDrawer({ node, activity, onClose }: NodeInspectorDrawerProps) {
  ...
  return (
    <>
      ...
      <aside
        className={clsx(
          "fixed inset-y-0 right-0 z-40 flex w-[360px] shrink-0 flex-col border-l border-slate-200 bg-white shadow-2xl transition-transform duration-300 ease-out",
          ...
          <>
            <div className="flex items-center gap-2.5 border-b border-slate-200 px-4 py-3.5">
              ...
              <div className="min-w-0 flex-1">
                <div className="truncate text-sm font-bold text-slate-800">{node.data.label}</div>
                <div className="truncate text-[11px] text-slate-400">{node.data.subtitle}</div>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="flex items-center justify-between gap-2 border-b border-slate-200 px-4 py-3">
              ...
              <div className="text-right">
                <span className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Latency</span>{" "}
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3.5">
              ...
              <pre className="mb-4 overflow-x-auto rounded-lg border border-slate-200 bg-slate-900 px-3 py-2.5 text-[11px] leading-relaxed text-emerald-300">
                {latest ? JSON.stringify(latest.payload, null, 2) : "// waiting for activity…"}
              </pre>

              <h3 className="mb-1.5 text-[11px] font-bold uppercase tracking-wide text-slate-400">Status Logs</h3>
              {activity.length === 0 ? (
                  ...
                  {activity.map((entry) => (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        <span className="truncate text-[11px] font-semibold text-slate-700">{entry.stepName}</span>
                        <span
                          className={clsx(
                            "shrink-0 text-[10px] font-bold",
                            entry.status === "success" ? "text-emerald-500" : "text-red-500",
                          )}
                        >
                          {entry.latencyMs}ms
                        </span>
